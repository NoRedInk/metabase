...
(def ^:private date-only-formatter (tformat/formatters :date))
(def ^:private iso-date-formatter (tformat/formatters :date-time))

(defn- datetime-str->date
  "Parse `date-str` as an iso formatted timestamp and emit the date-only portion of that timestamp"
  [date-str]
  (->> date-str
       (tformat/parse iso-date-formatter)
       (tformat/unparse date-only-formatter)))
